...
- If the site is instead published as a normal project Pages site, change `VITE_BASE_PATH` in `.github/workflows/deploy-pages.yml` to `/<repository-name>/` where `<repository-name>` is the repository slug segment (for example, `/stimulator/`), or remove the override and let Vite derive it automatically.
